...
#set.seed(86)
n <- 50; p <- 300 # number of observations and variables, tuning as wanted
...
eps <- 0.0; alpha0=3 # contamination level, tuning as wanted
...
rho1=0.95; rho2=0.05
for (i in 1:p1){ for(j in 1:p1) { sigma1[i,j]=rho^{abs(i-j)} } }
for (i in 1:p2){ for(j in 1:p2) { sigma2[i,j]=rho^{abs(i-j)} } }
...
# lasso.rmse <- sqrt(mean((y.test - lasso.predicted)^2))
 ...
 fit2<- lars(x.train, y.train, type="lasso", max.steps=200)
# 
...
fit2<-enet(x.train, y.train, lambda=0.5, max.steps=700)

beta_enet<-predict.enet(fit2, s=80, type="coef", mode="norm")$coef
