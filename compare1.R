...
eps <- 0.05 # contamination level, tuning as wanted
...
alpha=seq(0.01, 1, by=0.2)       #5 points, could be more, tuning
N_lars=100; alpha0=1; gam=1; n.folds=5 #allow LARS perform 100 times
...
# wi=get_weight_wi(test_x, test_y, beta_lst, alpha0) 
# indx_bt=which(wi==1); K =sum(wi)  
# pred<-test_x%*%beta_lst; rn=as.vector(test_y-pred)
...
# fit2<- lars(x.train, y.train, type="lasso", max.steps=100)
# 
# lasso.predicted <-predict(fit2, newx=x.test, s=10, type="fit")$fit
# 
# bt_lasso<-predict(fit2,  s=10, type="coefficients")
# 
# beta_lasso=bt_lasso$coef

fit3<-enet(x.train, y.train, lambda=0, max.steps=90)

beta_lasso<-predict.enet(fit3, s=0.45, type="coef", mode="fraction")$coef
...
#LARS
###############################
fit<- lars(x.train, y.train, type="lar", max.steps=100)
...
fit2<-enet(x.train, y.train, lambda=0.5, max.steps=90)

beta_enet<-predict.enet(fit2, s=0.45, type="coef", mode="fraction")$coef
...
m.lst=measure.lst_enet
m.lasso=measure.lasso
m.lars=measure.lars
m.enetLTS=measure.enetLTS
m.enet=measure.enet

par(mfrow = c(2,2))
boxplot(cbind(m.lst[,1], m.lasso[,1], m.lars[,1], m.enetLTS[,1], m.enet[,1]), 
         names=c("P1", "P2","P3", "P4", "P5"),
        xlab="5% contamination",
        ylab="L2-error",
        border="blue"
        )

boxplot(cbind(m.lst[,4], m.lasso[,4], m.lars[,4], m.enetLTS[,4], m.enet[,4]), 
        names=c("P1", "P2","P3", "P4", "P5"),
        xlab="5% contamination",
        ylab="RMSE",
        border="green"
)
boxplot(cbind(m.lst[,2], m.lasso[,2], m.lars[,2], m.enetLTS[,2], m.enet[,2]), 
        names=c("P1", "P2","P3", "P4", "P5"),
        xlab="5% contamination",
        ylab="TSDR",
        border="red"
